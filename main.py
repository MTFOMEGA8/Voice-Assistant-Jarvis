...
from database import init_db, get_owner_name, set_owner_name
...
def get_ai_response(prompt):
    ...
    try:
        completion = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": f"Ты — J.A.R.V.I.S., продвинутый ИИ. Твоего создателя зовут {user_name}. Отвечай кратко и по делу."},
...
user_name = get_owner_name()

if not user_name:
    msg = "Сэр, я не обнаружил вашего имени в базе данных. Пожалуйста, представьтесь."
    speak(msg) 
    
    recorded_name = listen() 
    
    if recorded_name:
        set_owner_name(recorded_name)
        user_name = recorded_name
        msg_success = f"Приятно познакомиться, {user_name}. Теперь я вас запомнил."
        speak(msg_success)
    else:
        user_name = "Сэр" 
else:
    greeting = f"Система активна. Приветствую, {user_name}!"
    speak(greeting)    
        
if __name__ == "__main__":
    speak("Протокол номер один точка три запущен. Голос и слух синхронизированы, сэр.")
